...
graphNetwork=`curl -s -X POST -H "Content-Type: application/json" -d '{ "query": "{graphNetworks { totalTokensStaked totalDelegatedTokens } } "}' https://api.thegraph.com/subgraphs/name/graphprotocol/graph-network-mainnet`
