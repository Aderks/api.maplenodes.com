...
graphNetwork=`curl -s -X POST -H "Content-Type: application/json" -d '{ "query": "{graphNetworks { totalTokensStaked totalDelegatedTokens } } "}' https://gateway.thegraph.com/network`
