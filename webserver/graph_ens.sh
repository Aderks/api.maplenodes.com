...
graphAccount=`curl -s -X POST -H "Content-Type: application/json" -d '{ "query": "{graphAccount(id: \"'$beneficiary'\") { defaultDisplayName } } "}' https://api.thegraph.com/subgraphs/name/graphprotocol/graph-network-mainnet`
