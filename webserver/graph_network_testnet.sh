...
graphNetwork=`curl -s -X POST -H "Content-Type: application/json" -d '{ "query": "{graphNetworks { totalTokensAllocated totalTokensStaked totalSupply totalDelegatedTokens } } "}' https://api.thegraph.com/subgraphs/name/graphprotocol/graph-network-goerli`
